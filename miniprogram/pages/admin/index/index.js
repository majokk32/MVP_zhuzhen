// pages/admin/index/index.js - 教师端任务列表（和学生端相同逻辑）
const app = getApp()
...
Page({
  ...
  async onLoad(options) {
    ...
    try {
      ...
      console.log(`教师端任务列表首屏加载完成: ${totalLoadTime}ms`);
      
    } catch (error) {
      console.error('教师端任务列表加载失败:', error);
      // 降级处理
  ...
  async loadTaskList(loadMore = false) {
    console.log('🎯 [DEBUG] 教师端loadTaskList 函数开始执行, loadMore:', loadMore);
  ...
  calculateListHeight() {
    wx.getSystemInfo({
      success: (res) => {
        ...
        // 教研管理按钮约 120rpx = 120/750 * windowWidth
        ...
        const adminBtnHeight = 120 * rpxRatio
        
        // 计算列表可用高度
        const listHeight = windowHeight - headerHeight - filterHeight - safeBottomHeight - adminBtnHeight - 20 // 20px预留空间
